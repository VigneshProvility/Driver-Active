...
function TripSheet() {
    ...
    // Fetch trip list when date changes
    useEffect(() => {
        try {
    ...
    return (
        <>
            ...
            <div className="trip-sheet col-11">
                {getUserInfoForReport().map((values, index) => (
                    <span key={index}>{values}</span>
                ))}
                <span
                    className="show-pointer"
                    onClick={() => downloadTripSheet(tripList)}
                >
          <FaDownload />
        </span>
            </div>
